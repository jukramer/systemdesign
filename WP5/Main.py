...
def calcVol(x):
    ...
    return vol, np.array([minMarginShear, minMarginComp, minMarginTens, np.min([minMarginShear,minMarginComp,minMarginTens]), theta[-1]*180/np.pi])
...
def optimise_main():
    ...
    constraints_sigma = sp.optimize.NonlinearConstraint(constraintWrap, lb=[1, 1, 1, 1, -10.0], ub=[np.inf, np.inf, np.inf, 1.1, 10.0])
